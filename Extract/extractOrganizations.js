...
var obj = JSON.parse(fs.readFileSync('Extracted_Entities_organizations.json', 'utf8'), 1000);
console.log(obj.length);
var count = 0
async.eachSeries(obj, function(item, cb){
    // item['extracted_Db'] = {}
    // item['extracted_wiki'] = {}
    console.log(item.name+': In Process');
    async.series([
        function(done) {
            if(item['rawData_Db']==null){
                console.log('Fetching dbpedia')
                var options = {  
                    url: 'http://lookup.dbpedia.org/api/search/PrefixSearch?QueryClass=&MaxHits=5&QueryString='+item.name,
                    method: 'GET',
                    headers: {
        ...
        function(done) {
            if(item['rawData_wiki:']==null){
                ...
                wikidataSearch.set('search', item.name); //set the search term
	    ...
	    if(count % 10 == 0 ) { 
		fs.writeFile('Extracted_Entities_organizations_mac.json', JSON.stringify(obj), 'utf8', function(err){
